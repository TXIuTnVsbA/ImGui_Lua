...
namespace lua {
	...
	namespace ns_ui {
		void add_window(std::string tab_name,int container,float w,float h) {
			int tabSize = 0; tabSize = GET_STRINGS[g_config.gui.tab_name].size();
			GET_STRINGS[g_config.gui.tab_name][tabSize + 1] = tab_name;
			GET_BOOLS_MAP[g_config.gui.tab_bool][tab_name] = false;
			GET_FLOATS_MAP[g_config.gui.w][tab_name] = w;
			GET_FLOATS_MAP[g_config.gui.h][tab_name] = h;
			GET_INTS_MAP[g_config.gui.container_count][tab_name] = container;
		}

		std::string add_text(sol::this_state s, std::string tab, std::string container, std::string label, std::string key) {
			MenuItem_t item;
	...
	void init_command() {
		...
		ui["add_window"] = ns_ui::add_window;
		lua_state["ui"] = ui;
